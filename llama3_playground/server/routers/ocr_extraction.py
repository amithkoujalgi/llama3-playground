...
from llama3_playground.core.utils import OCRInferenceRunner
...
async def run_ocr_async_pdf_yolo(file: UploadFile = File(...)):
    ...
    try:
        ...
        thread = Thread(
            name=ocr_run_id,
            target=OCRInferenceRunner.run_ocr_process_and_collect_result,
            kwargs={
                "run_id": ocr_run_id,
        ...
        if os.path.exists(status_file):
            ...
            if 'success' in status:
                ...
                if os.path.exists(text_result_file) and os.path.exists(ocr_result_file):
                    ...
                    #           'status': 'success'})
                    return ResponseHandler.success(data=ocr_result_dict)
                else:
