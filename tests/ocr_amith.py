...
class OCRByAmith:
    def __init__(self, results_dir: str, empty_results_dir_before_run: bool = False,
                 easyocr_model_storage_directory: str = None, easyocr_models_download_enabled: bool = True):
        self._intermediate_results_path = results_dir
        self._empty_results_dir_before_run = empty_results_dir_before_run
        if easyocr_model_storage_directory is None or easyocr_model_storage_directory.strip() == "":
            easyocr_model_storage_directory = os.path.join(str(Path.home()), 'easy-ocr-models')
            print(f"Using EasyOCR models dir: {easyocr_model_storage_directory}")
        self._easyocr_model_storage_directory = easyocr_model_storage_directory
        self._easyocr_models_download_enabled = easyocr_models_download_enabled

    def _pdf_to_images(self, pdf_path: str, pdf_pages_to_images_dir: str) -> []:
        """
    ...
    def _pdf_to_images2(self, pdf_path: str, pdf_pages_to_images_dir: str) -> []:
        """
    ...
    def _update_checkboxes_in_image(self, image_path):
    ...
    def print_dict_as_table(self, dict_to_print):
    ...
    def _run_ocr(self, image_file: str):
        ...
        _data = []
        reader = easyocr.Reader(['en'], model_storage_directory=self._easyocr_model_storage_directory,
                                download_enabled=self._easyocr_models_download_enabled)
    ...
    def process(self, pdf_path: str):
        ...
        if not os.path.exists(self._intermediate_results_path):
            os.makedirs(self._intermediate_results_path)

        print(f"Emptying dir: {self._intermediate_results_path}")

        pdf_pages_to_images_dir = f'{self._intermediate_results_path}/pdf-images'
        pdf_pages_to_text_dir = f'{self._intermediate_results_path}/pdf-ocr'
        ...
        images = self._pdf_to_images2(pdf_path=pdf_path, pdf_pages_to_images_dir=pdf_pages_to_images_dir)

        for i in range(len(images)):
            print(f"Updating checkboxes in page {i}...")
            self._update_checkboxes_in_image(image_path=images[i])
            print(f"Running OCR on page {i}...")
            page_txt = self._run_ocr(image_file=images[i])
        ...
        pages_ocr_data_file = os.path.join(self._intermediate_results_path, f'ocr-result.json')
        ...
        pages_text_data_file = os.path.join(self._intermediate_results_path, 'text-result.txt')
...
pdf_pages_text_data, pages_text_data_file_path, ocr_data, ocr_data_file_path = OCRByAmith(
    results_dir='/Users/amithkoujalgi/Downloads/llm-extraction/new2',
